...
void GeneralConfigPane::InitializeGUI()
{
	...
	m_continuous_play_checkbox   = new wxCheckBox(this, wxID_ANY, _("Continue from the last time"));
	...
	m_continuous_play_checkbox->SetToolTip(_("The games will automatically continue from the last time you've played them.\nAutomatically loads most recent savestate at game launch and automatically creates savestate then you close the game.\n\nWARNING: savestate are considered unstable by developer team, don't forget to save in-game."));
	...
	m_continuous_play_checkbox->Bind(wxEVT_CHECKBOX, &GeneralConfigPane::OnAutoLoadCheckBoxChanged, this);
	...
	basic_settings_sizer->Add(m_continuous_play_checkbox, 0, wxALL, 5);
...
void GeneralConfigPane::LoadGUIValues()
{
	...
	m_continuous_play_checkbox->SetValue(startup_params.bContinuousPlay);
...
{
	SConfig::GetInstance().bContinuousPlay = m_continuous_play_checkbox->IsChecked();
}
